...
pacman::p_load(operators,topmodel,DEoptim,soilDB,sp,curl,httr,
               rnoaa,raster,shapefiles,rgdal,elevatr,terra,progress,lubridate,heatwaveR)
LabNo="/HW10"
#
...
# 
setwd(mygitdir)
system(paste0("git config --global user.email 'cnicole00@vt.edu'")) 
...
table4_1_2=data.frame(
  ...
  pusle=c(.6,.6,.5,.5,.6,.7,.8,.9,.9),
  maxlen=c(1000,122,91,61,37,24,18,15,0)
)
...
DPTI05$MF2[(format(DPTI05$date,"%j") %in% c(121,213,274))]=5.4*10^-4
...
# Loop to solve MF and DF for new tau
for (i in 2:length(date)){
  if(MF2[i]<=MF2[i-1]){
    MF2[i]=MF2[i-1]*exp(-dt/tau2)-DF2[i-1]
  }
  DF2[i]=MF2[i]*(kF*MF2[i]*Rt[i]/(1+kF*MF2[i]*Rt[i]))
}
DPTI05$MF2=MF2
DPTI05$DF2=DF2
detach(DPTI05)

rm(list=c("MF","DF","MF2","DF2")) # Clean up the environment 
...
DPTI05$muS5= muTS_TI05*QS^((DPTI05$Tavg+5-TR)/10) # Eq. 5 adjusted for 5 degree increase
DPTI05$DS=(DPTI05$muS*MS_TI05*DPTI05$Rt)/10^6 # Eq. 4
DPTI05$DS5=(DPTI05$muS5*MS_TI05*DPTI05$Rt)/10^6 # Eq. 4 adjusted for 5 degree increase
plot(DPTI05$date,DPTI05$DS)

# Repeat for other TI classes
...
DPTI01$MF2[(format(DPTI01$date,"%j") %in% c(121,213,274))]=5.4*10^-4
attach(DPTI01)
for (i in 2:length(date)){
  if(MF[i]<=MF[i-1]){
    MF[i]=MF[i-1]*exp(-dt/tau)-DF[i-1]
...
DPTI01$muS5= muTS_TI01*QS^((DPTI05$Tavg+5-TR)/10)
DPTI01$DS5=(DPTI01$muS5*MS_TI01*DPTI01$Rt)/10^6
...
DPTI02$MF2[(format(DPTI02$date,"%j") %in% c(121,213,274))]=5.4*10^-4
attach(DPTI02)
for (i in 2:length(date)){
  if(MF[i]<=MF[i-1]){
    MF[i]=MF[i-1]*exp(-dt/tau)-DF[i-1]
...
DPTI02$muS5= muTS_TI02*QS^((DPTI02$Tavg+5-TR)/10)
DPTI02$DS5=(DPTI02$muS5*MS_TI02*DPTI02$Rt)/10^6
...
DPTI03$MF2[(format(DPTI03$date,"%j") %in% c(121,213,274))]=5.4*10^-4
attach(DPTI03)
for (i in 2:length(date)){
  if(MF[i]<=MF[i-1]){
    MF[i]=MF[i-1]*exp(-dt/tau)-DF[i-1]
...
DPTI03$muS5= muTS_TI03*QS^((DPTI03$Tavg+5-TR)/10)
DPTI03$DS5=(DPTI03$muS5*MS_TI03*DPTI03$Rt)/10^6
...
DPTI04$MF2[(format(DPTI04$date,"%j") %in% c(121,213,274))]=5.4*10^-4
attach(DPTI04)
for (i in 2:length(date)){
  if(MF[i]<=MF[i-1]){
    MF[i]=MF[i-1]*exp(-dt/tau)-DF[i-1]
...
DPTI04$muS5= muTS_TI04*QS^((DPTI04$Tavg+5-TR)/10)
DPTI04$DS5=(DPTI04$muS5*MS_TI04*DPTI04$Rt)/10^6
...
# HW01 solution
# To estimate the amount of P loss from a non-fertilized watershed, remove the DF term from the stream loss
DPLT$LT_nf=DPLT$LB + 
...
plot(DPLT$date,DPLT$LT_nf, type="l",ylim=c(1,10))

# HW02 solution
# find the average (mean) LT P loss
avg_loss <- mean(DPLT$LT) # kg/day

# calculate LT for 5 degree C temp increase
DPLT$LT5=DPLT$LB +
  TIC05$area*(DPTI05$DF + DPTI05$DS5)+
  TIC04$area*(DPTI04$DF + DPTI04$DS5)+
  TIC03$area*(DPTI03$DF + DPTI03$DS5)+
  TIC02$area*(DPTI02$DF + DPTI02$DS5)+
  TIC01$area*(DPTI01$DF + DPTI01$DS5)
plot(DPLT$date,DPLT$LT5, type="l",ylim=c(1,10), xlab="date", ylab="P loss in kg/day")
lines(DPLT$date,DPLT$LT, col="blue")

# find the average (mean) LT5 P loss
avg_loss5 <- mean(DPLT$LT5)

# HW03 solution
# calculate LT for tau change to 2
DPLT$LT2=DPLT$LB +
  TIC05$area*(DPTI05$DF2 + DPTI05$DS)+
  TIC04$area*(DPTI04$DF2 + DPTI04$DS)+
  TIC03$area*(DPTI03$DF2 + DPTI03$DS)+
  TIC02$area*(DPTI02$DF2 + DPTI02$DS)+
  TIC01$area*(DPTI01$DF2 + DPTI01$DS)
plot(DPLT$date,DPLT$LT2, type="l",ylim=c(1,10), xlab="date", ylab="P loss in kg/day")
lines(DPLT$date,DPLT$LT, col="blue")

# find the average (mean) LT2 P loss
avg_loss2 <- mean(DPLT$LT2)
