...
HWNo="/HW07"
myflowgage_id="01208950"  # Old Friendly Gage
#
...
upper <- c(98,0.20, 0.05, 0.1, 0.4, 2000,   0.5)
...
# not adjusted for up-to-date measurements of streamflow
dat[is.na(dat)]=0
View(dat)
head(dat)
...
net <- neuralnet(Qm3ps ~ Qm3ps+Precip+jdate+Jday+TMX+TMN,
                 data = train,
                 hidden = c(6, 3), # can adjust here
                 threshold = 0.1, # if you network does not run, 
...
nn_prediction = predict(net, train[
  ,c("Qm3ps","Precip","jdate","Jday","TMX","TMN")])
...
nn_prediction = predict(net, test[, c("Qm3ps","Precip","jdate","Jday","TMX","TMN")])
