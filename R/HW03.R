...
# to get the distance
searchlength=sqrt(myflowgage$area*8)*1000  #maybe have to do 8.1 if it doesn't quite fit
...
myflowgage$area/(res(mydem)[1])^2*1000000 
#make this the thresh value in line 208 and 236, but then play around with it
...
# "Create AOI from a zipped shapefile"

url="https://websoilsurvey.sc.egov.usda.gov/DSD/Download/AOI/drleg30ka5y3xxq2g2um3tn0/wss_aoi_2023-02-15_18-17-28.zip"
...
mysoil=readOGR("wss_aoi_2023-02-15_18-17-28/spatial/soilmu_a_aoi.shp") 
...
#run it again with different bmlt values
...
TMWB$dP = TMWB$P-TMWB$SNO-TMWB$ET + TMWB$SNOmlt 
...
# Make a plot of ET, AW, Excess, Qpred, Qmm, and Precipitation
...
# A few constants
precipColor <- "#69b3a2"
  ETColor <- rgb(0.2, 0.6, 0.9, 1)
  
  ggplot(TMWB, aes(x=date)) +
    
    geom_line( aes(y=ET), size=2, color=ETColor) + 
    geom_line(aes(y=AW),size=2,color="blue")+
    geom_line(aes(y=Excess),size=2,color="red")+
    geom_line(aes(y=Qpred),size=2,color="black")+
    geom_line(aes(y=Qmm),size=2,color="orange")
    geom_line( aes(y=P), size=2, color=precipColor) +
    
    scale_y_continuous(
      
      # Features of the first axis
      name = "ET, AW, Excess, Precipitation (in)",
      
      # Add a second axis and specify its features
      sec.axis = sec_axis(~.*coeff,name="Qpred, Qmm")) +
      
    ggtitle("Thornthwaite-Mather Water Balance at Sasco Brook Near Southport, CT")+
      
    theme_ipsum() +
    
    theme(
      axis.title.y = element_text(color = ETColor, size=13),
      axis.title.y.right = element_text(color = "black", size=13))

#NSE test of model accuracy
Yobs<-TMWB$Qmm
Ysim<-TMWB$Qpred
