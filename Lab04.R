...
mydem=get_aws_terrain(locations=bboxpts@coords, 
                      z = 10, prj = proj4_utm,src ="aws",expand=1) # change z to make it faster by decreasing resolution
res(mydem)
plot(mydem)
plot(bboxpts,add=T)
...
myflowgage$area * 10^3 * 10^3 / res(mydem)[1]^2 / targetbasins #get your threshold - will be used later
subthreshold=myflowgage$area * 10^3 * 10^3 / res(mydem)[1]^2 / targetbasins
subthreshold=as.integer(subthreshold)
# Threshold
syscmd=paste0("mpiexec -n 2 threshold -ssa mydemad8.tif -src mydemsrc.tif -thresh ",subthreshold) #telling TAUDem threshold command how many to set before it starts a new string
system(syscmd)
...
# gdal_polygonize.py -8 mydemw.tif mydemw_poly_gdal.shp this is already in terra
mydemw=rast("mydemw.tif") #bring it in as a terra object, should come in as a spatial raster (spatrast)
...
# should result in a SpatVector with a number of polygons, number in values is the number of basins
...
#
# associate mukeys with cokeys from NRCS soils database
...
soilmap=merge(ssurgo.geom_utm_crop,mu2chmax,all=T) # merge crop soil map with much, merge common attributes mchmax table has cokey and mukey as well, so we can color things based on attributes

par(mfrow=c(2,2))
plot(soilmap,y="ksat_r")
plot(soilmap,y="awc_r")
plot(soilmap,y="hzdepb_r") #horizon depth
plot(mydemw_poly,main="Subbasins",col=rainbow(length(mydemw_poly)))
#can run this whole chunk again after filling in with means
...
# fill missing data with the mean of the saturated conductivities
...
TI = log( (sca+1)/(slp+0.00001) ) #topographic index - log of contributing area (log because you wouldn't actually be able to see the streams) divided by the slope
plot(TI)
...
# 
TI=raster(TI_terra)
pacman::p_load(classInt) # determine how you want the breaks to actually happen
...
plot(TIC) #highest TI green areas are streams, hillslopes are white, bc of low contributing area and not low slope
TIC_terra=rast(TIC)
plot(TIC_terra)
